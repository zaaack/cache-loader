...
function loader(...args) {
  ...
  const toDepDetails = (dep, mapCallback) => {
    fs.stat(dep, (err, stats) => {
      ...
      const mtimeValue = stats.mtime.getTime();
      if (mtimeValue / 1000 >= Math.floor(data.startTime / 1000)) {
        // Don't trust mtime. File was changed while compiling or it could be an inaccurate filesystem.
        allowCache = false;
      }
      mapCallback(null, {
        path: dep,
